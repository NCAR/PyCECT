    ...
    #     if no gathered file exists check that individual means files exist
        ...
        if len(mean_files) > 0:
            ...
            for i in range(len(mean_files)):
                gathered_mpas_means[i, :, :] = np.load(
                    folder_name + '/mpas_means_' + str(i) + '.npy'
                )
...
def read_CESM_ensemble_means(folder_name):
    ...
    #     if no gathered file exists check that indiviual means files exist
        ...
        if len(mean_files) > 0:
            ...
            for i in range(len(mean_files)):
                gathered_mpas_means[i, :, :] = np.load(
                    folder_name + '/cesm_means_' + str(i) + '.npy'
                )
...
def ECT_test_step(
    ...
    if test_vals.shape[0] != N_new:
        ...
        return (
            fail_bool,
            np.sum(test_sigma_fails, axis=0),
            np.abs(np.sum(test_sigma_fails, axis=0) @ np.linalg.inv(PC_vectors.T)),
        )
