# This file contain library functions needed for the ECT setup framework in as described in Price-Broncucia et al. 2024
...
from helper_ect_functions import (
    ECT_ensemble_step,
    ECT_test_step,
    read_CESM_ensemble_means,
    read_MPAS_ensemble_means,
)
...
def read_summary_series(folder, file_prefix, file_suffix, starting_timestep, save_interval):
    ...
    for i in range(file_count):
        datasets.append(
            xr.open_dataset(f'{folder}/{file_prefix}{i}{file_suffix}', engine='netcdf4')
        )
    ...
    for i in range(file_count):
        ...
        idx_of_shared_in_dataset = sorter[
            np.searchsorted(datasets[i].vars, shared_vars, sorter=sorter)
        ]

        all_standard_means[i, :, :] = xr.open_dataset(
            f'{folder}/{file_prefix}{i}{file_suffix}'
        ).standardized_gm[idx_of_shared_in_dataset, :]

    timesteps = list(
        range(starting_timestep, file_count * save_interval + starting_timestep, save_interval)
    )
...
def shapiro_wilks_over_time(
    ...
    non_normal_vars = np.sum(shap_ps < 0.05, axis=0)
    ...
    if mark_timestep is not None:
        plt.axvline(
            x=mark_timestep, label=f'Timestep: {mark_timestep}', color='red', linestyle='dashed'
        )
        plt.legend()
...
def plot_shapiro_wilks_over_time(
    timesteps, non_normal_vars_percentage, title='', mark_timestep=None
):
    plt.plot(timesteps, non_normal_vars_percentage)
    plt.xlabel('Model Timestep')
    plt.xticks(timesteps[::2], map(str, timesteps[::2]))
...
def var_explained_at_fixed_pc(
    folder, file_prefix, file_suffix, timestep, all_timesteps, samples, pcs
):
    idx = timestep_to_idx(timestep, all_timesteps)
...
def find_theoretical_m(PC_count, goal_FPR):
    sol = root_scalar(theoretical_fpr_for_opt, args=(PC_count, goal_FPR), bracket=[2.0, 5.0])
...
def fpr(standardized_gms, ensemble_size, test_repeats, PC_count, m_sigma):
    ...
    for j in range(test_repeats):
        ...
        fail_bool, sigma_fail, _ = ECT_test_step(
            test_samples.T,
            mean_shift,
            scale_multiply,
            PC_vectors,
...
def plot_fpr_tests(sample_sizes, fpr_by_ensemble_size, title, vertical_line=None):
    plt.axhline(y=0.005, color='red', linestyle='dashed', label='Goal FPR=0.5%')
    if vertical_line is not None:
        plt.axvline(x=vertical_line, linestyle='dashed', color='red')
...
def plot_continuous_test_results(data_path, scale='log'):
    ...
    test_data['perturbations'] = test_data.direction * 10.0 ** (test_data.order)
    ...
    for param in tested_parameters:
        ...
        tmp_true = True
        test_fail_indices = sorted_data_slice.index[sorted_data_slice.fail == tmp_true]
        ...
        plt.plot(filtered_x, filtered_y, alpha=0.2, linestyle='dashed')
        plt.axvline(x=0, color='b', label='Default Variable Value')

        for fail_idx in test_fail_indices:
            plt.scatter(
                x=x[fail_idx], y=y[fail_idx], color='r', marker='x', label='Model Run Failure'
            )
...
def fail_rates(means, test_repeats, ensemble_size, all_timesteps, single_timestep, m_sigma):
    ...
    for j in range(test_repeats):
        ...
        fail_bool, pc_fail, var_fail = ECT_test_step(
            test_samples,
            mean_shift,
            scale_multiply,
            PC_vectors,
...
def fpr_by_pc_and_var(
    means, test_repeats, ensemble_repeats, ensemble_size, all_timesteps, single_timestep, m_sigma
):
    ...
    for i in range(ensemble_repeats):
        #         print(i)
        test_fails, pc_fails, var_fails = fail_rates(
            means, test_repeats, ensemble_size, all_timesteps, single_timestep, m_sigma
        )
...
def variance_explained_vs_ensemble_size_plot(
    means, test_repeats, ensemble_sizes, all_timesteps, single_timestep, title
):
    timestep_count, var_count, total_ensemble = means.shape
    idx = timestep_to_idx(single_timestep, all_timesteps)
    ...
    for i, ensemble_size in enumerate(ensemble_sizes):
        scores_variances, cum_sum = var_explained_sample(
            means[idx, :, :], ensemble_size, test_repeats
        )
