...
def main(argv):
    ...
    for each in test_vars:
        ...
        perturbations = np.append(
            -(10 ** np.array(neg_test_orders, dtype=float)),
            10 ** np.array(pos_test_orders, dtype=float),
        )
        ...
        for i, order in enumerate(neg_test_orders):
            ...
            if os.path.isfile(f'{test_output_dir}/{test_folder}/eet.npy'):
                print(f'EET file found for {var_name}_perturb_neg{order}')
            ...
            # check if fail file is found
                ...
                print(
                    f'''Neither EET file nor Model run failure file found found for {var_name}_perturb_neg{order} \n
        ...
        for i, order in enumerate(pos_test_orders):
            ...
            # check if pca file exists and read in pca and eet files if so
            if os.path.isfile(f'{test_output_dir}/{test_folder}/eet.npy'):
                print(f'EET file found for {var_name}_perturb_{order}')
            ...
            # check if fail file is found
                ...
                print(
                    f'''Neither EET failure file nor Model run failure file found found for {var_name}_perturb_{order} \n
                      Did you run the post_run_script.py?'''
                )

        # sort outputs in order so they plot correctly
        # perturbations, test_vals, avg_pca_fails, avg_eet_fails = map(
        #     np.array, zip(*sorted(zip(perturbations, test_vals, avg_pca_fails, avg_eet_fails)))
        # )
        perturbations, test_vals, avg_eet_fails = map(
            np.array, zip(*sorted(zip(perturbations, test_vals, avg_eet_fails)))
        )
        ...
        for perturb_arg in [True, False]:
            for log_arg in ['log', 'linear']:
                plot_test_results(
                    ...
                    test_output_dir,
                    # plot_pca=pca_arg,
                    plot_perturbations=perturb_arg,
                    scale=log_arg,
                )
...
# def plot_test_results(plot_data, file_path, scale='log', plot_pca=True, plot_perturbations=True):
    ...
    if plot_perturbations:
        ...
        plt.xticks(
            test_vals_with_default - default_val,
            labels=['%.5g' % z for z in test_vals_with_default],
            rotation=50,
        )
