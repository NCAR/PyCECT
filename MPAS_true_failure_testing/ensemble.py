...
def main(argv):
    ...
    parser.add_argument('-rd', '--run_dir', default = None, dest='run_dir_root', help='Directory to create run\
    directories in (default: None).', required = True)
    parser.add_argument('-c', '--control_run', default = None , dest='control_run', help = 'Path to the control run\
    that will be duplicated for the ensemble members (default = None).', required = True)
    parser.add_argument('-v', '--verify', default=False, dest = 'verify', action = 'store_true', help = 'To create \
    random verifications runs (instead of an ensemble).')   
    parser.add_argument('-es', '--ens_size', default = 10, dest='ens_size', help = 'Total number of ensemble \
    members (default = 10).', type = int)
    parser.add_argument('-vs', '--verify_size', default = 3, dest='verify_size', help = 'Total number of ensemble \
    members (default = 3).', type = int) 
    parser.add_argument('-st', '--start', default = 0, dest = 'ens_start', help = 'Ensemble run to start with\
    (default = 0).', type = int)
    parser.add_argument('-rs', '--run_script', default = 'chey-run.sh', dest='run_script', help='Run script name to\
    submit to PBS queue in the control run directory (default: chey-run.sh).')
    parser.add_argument('-s', '--submit', default = False, dest = 'submit' , action = 'store_true', help='Indicates\
    ...
    if run_size > 0:
        if run_size > 10000:
            print("Error: cannot have an ensemble size greater than 10000.")
            sys.exit()
    ...
    if not os.path.exists(run_dir_root):
        print("ERROR: the specified directory for the new runs  (", run_dir_root, ") does not exist. Exiting")
        sys.exit()
    ...
    for i in range(ens_start, run_size):                                              
        ...
        ret = os.chdir(run_dir_root)
        command = "cp -r " + control_run + " " + new_run
        ret = os.system(command)
        ret = os.chdir(new_run)
        thisdir = os.getcwd()
